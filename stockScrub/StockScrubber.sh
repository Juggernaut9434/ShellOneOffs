...
# sed allows multiple expressions that substitutes
# the `g` allows for all matches for that line otherwise first occurence
#
cat results.txt | tr -d [a-zA-Z\<\/\>\"=] | tr '\n' ',' | tr -d [:space:] |
sed -e 's/,,,/,/' -e 's/,,/,/g' -e 's/,,/,\n,/g' > results.txt

# put ticker in front of the line
echo "" > temp.txt  # clean temp file with empty first line
c=1
for i in ${TICKERS[@]}
do
